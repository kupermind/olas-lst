...
interface IStaking {
    ...
    /// @dev Gets service proxy staking token.
    /// @return Service proxy staking token address.
    function stakingProxyToken() external view returns (address);
...
contract StakingVerifier {
    ...
    /// @param _proxyOlas Proxy OLAS token address.
    ...
    constructor(
        ...
        // Zero address check
        if (_olas == address(0) || _proxyOlas == address(0) || _serviceRegistry == address(0)) {
            revert ZeroAddress();
        }
