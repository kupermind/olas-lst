...
withdrawDelay=$(jq -r '.withdrawDelay' $globals)
proxyData=$(cast calldata "initialize(uint256)" $withdrawDelay)
...
constructorArgs="$treasuryAddress $proxyData"
...
treasuryProxyAddress=$(echo "$deploymentOutput" | grep 'Deployed to:' | awk '{print $3}')

# Get output length
outputLength=${#treasuryProxyAddress}
...
echo "$(jq '. += {"treasuryProxyAddress":"'$treasuryProxyAddress'"}' $globals)" > $globals

# Verify contract
if [ "$contractVerification" == "true" ]; then
  contractParams="$treasuryProxyAddress $contractPath --constructor-args $(cast abi-encode "constructor(address,bytes)" $constructorArgs)"
...
echo "${green}$contractName deployed at: $treasuryProxyAddress${reset}"
