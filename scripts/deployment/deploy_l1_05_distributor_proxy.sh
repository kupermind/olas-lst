...
distributorAddress=$(jq -r '.distributorAddress' $globals)
lockFactor=$(jq -r '.lockFactor' $globals)
proxyData=$(cast calldata "initialize(uint256)" $lockFactor)
...
distributorProxyAddress=$(echo "$deploymentOutput" | grep 'Deployed to:' | awk '{print $3}')

# Get output length
outputLength=${#distributorProxyAddress}
...
echo "$(jq '. += {"distributorProxyAddress":"'$distributorProxyAddress'"}' $globals)" > $globals

# Verify contract
if [ "$contractVerification" == "true" ]; then
  contractParams="$distributorProxyAddress $contractPath --constructor-args $(cast abi-encode "constructor(address,bytes)" $constructorArgs)"
...
echo "${green}$contractName deployed at: $distributorProxyAddress${reset}"
