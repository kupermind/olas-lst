...
constructorArgs="$olasAddress $veOLASAddress"
...
lockAddress=$(echo "$deploymentOutput" | grep 'Deployed to:' | awk '{print $3}')

# Get output length
outputLength=${#lockAddress}
...
echo "$(jq '. += {"lockAddress":"'$lockAddress'"}' $globals)" > $globals

# Verify contract
if [ "$contractVerification" == "true" ]; then
  contractParams="$lockAddress $contractPath --constructor-args $(cast abi-encode "constructor(address,address)" $constructorArgs)"
...
echo "${green}$contractName deployed at: $lockAddress${reset}"
