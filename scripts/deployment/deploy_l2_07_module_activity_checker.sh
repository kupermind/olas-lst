...
contractName="ModuleActivityChecker"
...
moduleActivityCheckerAddress=$(echo "$deploymentOutput" | grep 'Deployed to:' | awk '{print $3}')

# Get output length
outputLength=${#moduleActivityCheckerAddress}
...
echo "$(jq '. += {"moduleActivityCheckerAddress":"'$moduleActivityCheckerAddress'"}' $globals)" > $globals

# Verify contract
if [ "$contractVerification" == "true" ]; then
  contractParams="$moduleActivityCheckerAddress $contractPath --constructor-args $(cast abi-encode "constructor(uint256)" $constructorArgs)"
...
echo "${green}$contractName deployed at: $moduleActivityCheckerAddress${reset}"
