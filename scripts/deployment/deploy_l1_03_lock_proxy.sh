...
lockAddress=$(jq -r '.lockAddress' $globals)
...
constructorArgs="$lockAddress $proxyData"
...
lockProxyAddress=$(echo "$deploymentOutput" | grep 'Deployed to:' | awk '{print $3}')

# Get output length
outputLength=${#lockProxyAddress}
...
echo "$(jq '. += {"lockProxyAddress":"'$lockProxyAddress'"}' $globals)" > $globals

# Verify contract
if [ "$contractVerification" == "true" ]; then
  contractParams="$lockProxyAddress $contractPath --constructor-args $(cast abi-encode "constructor(address,bytes)" $constructorArgs)"
...
echo "${green}$contractName deployed at: $lockProxyAddress${reset}"
