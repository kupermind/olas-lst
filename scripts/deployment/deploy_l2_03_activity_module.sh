...
activityModuleAddress=$(echo "$deploymentOutput" | grep 'Deployed to:' | awk '{print $3}')

# Get output length
outputLength=${#activityModuleAddress}
...
echo "$(jq '. += {"activityModuleAddress":"'$activityModuleAddress'"}' $globals)" > $globals

# Verify contract
if [ "$contractVerification" == "true" ]; then
  contractParams="$activityModuleAddress $contractPath --constructor-args $(cast abi-encode "constructor(address,address,address)" $constructorArgs)"
...
echo "${green}$contractName deployed at: $activityModuleAddress${reset}"
