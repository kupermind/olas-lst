...
stOLASAddress=$(echo "$deploymentOutput" | grep 'Deployed to:' | awk '{print $3}')

# Get output length
outputLength=${#stOLASAddress}
...
echo "$(jq '. += {"stOLASAddress":"'$stOLASAddress'"}' $globals)" > $globals

# Verify contract
if [ "$contractVerification" == "true" ]; then
  contractParams="$stOLASAddress $contractPath --constructor-args $(cast abi-encode "constructor(address)" $constructorArgs)"
...
echo "${green}$contractName deployed at: $stOLASAddress${reset}"
