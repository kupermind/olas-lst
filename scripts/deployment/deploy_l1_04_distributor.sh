...
constructorArgs="$olasAddress $stOLASAddress $lockProxyAddress"
...
distributorAddress=$(echo "$deploymentOutput" | grep 'Deployed to:' | awk '{print $3}')

# Get output length
outputLength=${#distributorAddress}
...
echo "$(jq '. += {"distributorAddress":"'$distributorAddress'"}' $globals)" > $globals

# Verify contract
if [ "$contractVerification" == "true" ]; then
  contractParams="$distributorAddress $contractPath --constructor-args $(cast abi-encode "constructor(address,address,address)" $constructorArgs)"
...
echo "${green}$contractName deployed at: $distributorAddress${reset}"
