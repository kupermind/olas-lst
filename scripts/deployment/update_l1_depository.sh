...
constructorArgs="$olasAddress $stOLASAddress"
...
depositoryAddress=$(echo "$deploymentOutput" | grep 'Deployed to:' | awk '{print $3}')

# Get output length
outputLength=${#depositoryAddress}
...
echo "$(jq '. += {"depositoryAddress":"'$depositoryAddress'"}' $globals)" > $globals

# Verify contract
if [ "$contractVerification" == "true" ]; then
  contractParams="$depositoryAddress $contractPath --constructor-args $(cast abi-encode "constructor(address,address)" $constructorArgs)"
...
echo "${green}Contract deployed at: $depositoryAddress${reset}"

echo "${green}Updating implementation${reset}"
castSendHeader="cast send --rpc-url $networkURL$API_KEY $walletArgs"
castArgs="$depositoryProxyAddress changeImplementation(address) $depositoryAddress"
echo $castArgs
