...
constructorArgs="$activityModuleAddress"
...
beaconAddress=$(echo "$deploymentOutput" | grep 'Deployed to:' | awk '{print $3}')

# Get output length
outputLength=${#beaconAddress}
...
echo "$(jq '. += {"beaconAddress":"'$beaconAddress'"}' $globals)" > $globals

# Verify contract
if [ "$contractVerification" == "true" ]; then
  contractParams="$beaconAddress $contractPath --constructor-args $(cast abi-encode "constructor(address)" $constructorArgs)"
...
echo "${green}$contractName deployed at: $beaconAddress${reset}"
