...
unstakeRelayerAddress=$(jq -r '.unstakeRelayerAddress' $globals)
...
constructorArgs="$unstakeRelayerAddress $proxyData"
...
unstakeRelayerProxyAddress=$(echo "$deploymentOutput" | grep 'Deployed to:' | awk '{print $3}')

# Get output length
outputLength=${#unstakeRelayerProxyAddress}
...
echo "$(jq '. += {"unstakeRelayerProxyAddress":"'$unstakeRelayerProxyAddress'"}' $globals)" > $globals

# Verify contract
if [ "$contractVerification" == "true" ]; then
  contractParams="$unstakeRelayerProxyAddress $contractPath --constructor-args $(cast abi-encode "constructor(address,bytes)" $constructorArgs)"
...
echo "${green}$contractName deployed at: $unstakeRelayerProxyAddress${reset}"
