...
unstakeRelayerAddress=$(echo "$deploymentOutput" | grep 'Deployed to:' | awk '{print $3}')

# Get output length
outputLength=${#unstakeRelayerAddress}
...
echo "$(jq '. += {"unstakeRelayerAddress":"'$unstakeRelayerAddress'"}' $globals)" > $globals

# Verify contract
if [ "$contractVerification" == "true" ]; then
  contractParams="$unstakeRelayerAddress $contractPath --constructor-args $(cast abi-encode "constructor(address,address)" $constructorArgs)"
...
echo "${green}$contractName deployed at: $unstakeRelayerAddress${reset}"
